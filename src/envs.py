...
def get_env_info(env:gym.Env,env_id: str ="unknow")-> EnvInfo:
    obs_space=env.observation_space
    act_space=env.action_space

    if not hasattr(obs_space, "shape") or obs_space is None:
        raise ValueError("Observation space must have a valid shape!!!")
    observation_shape=obs_space.shape

    if len(observation_shape)!=1 :
        raise ValueError(
            f"Expected 1D observation space for LunarLander, got {observation_shape}."
        )
    ...
    elif hasattr(act_space,"shpae") and act_space.shape is not None:
